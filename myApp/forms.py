...
from .models import Location, Person, Profile, Crop_Feasibility, FCT, Crop_SubNational, Countries, Crop_Name, Crop_Feasibility_instant
from django.contrib.auth.models import User
...
class Crop_Feas2_Form(forms.ModelForm):
  crop_name = forms.CharField(max_length=200)
  crop_name_id = forms.IntegerField()
  ...
  def clean(self):
    cleaned_data = super(Crop_Feas2_Form, self).clean()
    self.cleaned_data['myFCT'] = FCT.objects.get(food_item_id=self.cleaned_data['crop_name_id'])
    tmp_nut = round(int(self.cleaned_data['feas_DRI_e']) * 10 / 3)
    tmp_soc = round((int(self.cleaned_data['feas_soc_acceptable']) + int(
      self.cleaned_data['feas_soc_acceptable_c5']) + int(self.cleaned_data['feas_soc_acceptable_wo']) + int(
    ...
    logger.info(self.cleaned_data['crop_score'])
    logger.info(self.cleaned_data['myFCT'])
    logger.error('now these are outputs')
    for ele in self.cleaned_data:
      logger.info(ele)
